...
# UPDATED: Removes AI-generated source lists to prevent duplication.
...
class AdvancedContentValidator:
    ...
    def sanitize_toc_links(self, html_content):
        ...
        if re.search(pattern, html_content):
            logger.warning("🔧 Detected broken TOC links. Scrubbing...")
            html_content = re.sub(pattern, r'href="#\1"', html_content)
        return html_content

    # --- NEW: REMOVE AI SOURCES ---
    def remove_redundant_sources(self, html_content):
        """
        Removes 'Sources', 'References', or 'Bibliography' sections generated by AI
        to avoid duplication with our hard-coded Python sources block.
        """
        soup = BeautifulSoup(html_content, 'html.parser')
        # Keywords that indicate a sources section
        targets = ['sources', 'references', 'bibliography', 'citations']
        
        # Find headers (h2, h3, h4) containing these words
        for header in soup.find_all(['h2', 'h3', 'h4', 'h5']):
            text = header.get_text().strip().lower()
            if text in targets or text == "sources & references":
                logger.info(f"🧹 Removing redundant AI section: {text}")
                # Remove the header
                header.decompose()
                # Remove the list immediately following it (ul or ol)
    ...
    def run_professional_validation(self, html_content, full_source_text, sources_metadata):
        ...
        # 1. Remove AI Sources (To avoid duplicates)
        html = self.remove_redundant_sources(html_content)

        # 2. Fix TOC
        html = self.sanitize_toc_links(html)
