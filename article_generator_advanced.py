...
def decode_google_news_url(source_url):
    """
    Decodes 'https://news.google.com/rss/articles/...' to real URL.
    This bypasses the need to wait for Google's JS redirection.
    """
    try:
        # Extract base64 part
        ...
        if pad: base64_str += '=' * (4 - pad)

        decoded_bytes = base64.urlsafe_b64decode(base64_str)
        decoded_str = decoded_bytes.decode('latin1', errors='ignore')

        # Regex to find hidden URL
        # Looks for http:// or https:// followed by typical domain chars
...
def fetch_full_article(url):
    """
    SMART SCRAPER (v3.0): Decodes Google links + Timeouts
    """
    # 1. Resolve URL FIRST (Skip the middleman)
    target_url = decode_google_news_url(url)
    
    log(f"   🕷️ Scraping: {target_url}")
    ...
    try:
        ...
        if len(full_text) < 500:
            log("      ⚠️ Content too short (Protection/JS only).")
            return None
            
        log(f"      ✅ Fetched {len(full_text)} chars.")
        return full_text
...
def get_real_news_rss(query_keywords, category):
    try:
        ...
        items = []
        if feed.entries:
            # We take 8 items to have buffer for scraping failures
...
# ==============================================================================
# 5. CORE PIPELINE LOGIC (SCRAPING ENHANCED)
# ==============================================================================
...
def run_pipeline(category, config, mode="trending"):
    ...
    # Loop to find Scapable Article
    selected_item = None
    source_content = None
    
    for item in rss_items[:3]: # Try top 3
        if item['title'][:30] in recent: continue
        
        # New Scrape with Decoder
        text = fetch_full_article(item['link'])
        
        if text and len(text) > 500:
            selected_item = item
            source_content = text
            log("   ✅ Valid content scraped!")
            break
    ...
    if not selected_item:
        if rss_items:
            log("⚠️ Warning: Scraping all failed. Using Analyst Mode (Headline Only).")
            selected_item = rss_items[0]
            is_analyst = True
            source_content = "SOURCE CONTENT UNAVAILABLE. PLEASE ANALYZE BASED ON HEADLINE/KNOWLEDGE."
        else: return
    ...
    # Pass 2: Write Draft
    prefix = "*** SOURCE TEXT FOLLOWS ***" if not is_analyst else "*** ANALYST MODE - NO SOURCE ***"
