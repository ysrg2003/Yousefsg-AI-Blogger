...
# Please paste your detailed "Beast Mode" Prompts here as agreed.
...
def decode_google_news_url(source_url):
    """
    Decodes 'https://news.google.com/rss/articles/...' into the real destination URL.
    This fixes the '0 chars' scraping issue by skipping the Google redirect page completely.
    """
    try:
        # Extract the Base64 ID part after 'articles/'
        # Example: .../articles/CBMiUW... -> "CBMiUW..."
        ...
        decoded_str = decoded_bytes.decode('latin1') # Decoding as raw chars to regex

        # Find the URL hidden inside. It typically starts with https://
        # and allows a sequence of typical URL characters.
        # This regex looks for the first http(s) URL inside the binary mess.
        urls = re.findall(r'(https?://[a-zA-Z0-9./_\-%]+)', decoded_str)
        
        for url in urls:
            # Filter out google own links
            if "google.com" not in url and "." in url:
                log(f"      🔓 Decoded Real URL: {url}")
                return url
        
        return source_url # Return original if decode fails
    except:
        return source_url

def fetch_full_article(url):
    """
    UPGRADED SCRAPER (v3.0): Includes Decoder for Google News Links.
    """
    # 1. Resolve Real URL first!
    real_url = decode_google_news_url(url)
    
    log(f"   🕷️ Scraping: {real_url}")
    ...
    try:
        ...
        # Follow redirects in case there's another layer (e.g. t.co or bit.ly)
        response = session.get(real_url, headers=headers, timeout=20, allow_redirects=True)
        ...
        # Extract substantial text
        paragraphs = [p.get_text().strip() for p in soup.find_all('p') if len(p.get_text()) > 60]
        
        full_text = "\n\n".join(paragraphs)[:15000]
        
        if len(full_text) < 500:
            log("      ⚠️ Text too short. Probably a JS-Heavy site.")
            return None
            
        log(f"      ✅ Captured {len(full_text)} characters.")
        return full_text
...
def get_real_news_rss(query_keywords, category):
    try:
        ...
        if feed.entries:
            ...
            log(f"   ⚠️ RSS Empty. Trying broad category search.")
            fallback = f"{category} news when:1d"
            url = f"https://news.google.com/rss/search?q={urllib.parse.quote(fallback)}&hl=en-US&gl=US&ceid=US:en"
            feed = feedparser.parse(url)
...
# ==============================================================================
# 5. PIPELINE LOGIC (RSS + DECODE + SCRAPE + 5 STEPS)
# ==============================================================================
...
def run_pipeline(category, config, mode="trending"):
    ...
    for item in rss_items[:3]: # Check 3 Candidates
        if item['title'][:20] in recent: continue # Anti-duplication
        
        # Scrape with Decoding Power
    ...
    if not selected_item:
        if rss_items:
            log("⚠️ Fallback: No content could be scraped. Using Headline Analysis Mode.")
            selected_item = rss_items[0]
            is_analyst = True
            source_content = "SOURCE TEXT UNAVAILABLE. Use General Knowledge to analyze this Headline."
        else: return
    ...
    prefix = "*** SOURCE FULL TEXT ***" if not is_analyst else "*** HEADLINE ONLY MODE ***"
    payload = f"METADATA: {json.dumps(json_ctx)}\n\n{prefix}\n\n{source_content}"
    
    # Step B: Writer (Using Source)
    log(f"   🗞️ Writing Topic: {selected_item['title']}")
    ...
    # Step C: SEO
    kg = get_relevant_kg_for_linking(category)
    prompt_c = PROMPT_C_TEMPLATE.format(json_input=json.dumps(json_b), knowledge_graph=kg)
    ...
    vid_html, vid_main, vid_short, fb_vid_path = "", None, None, None
    try:
    ...
    if url:
        ...
        try:
            if fb_vid_path: social_manager.post_reel_to_facebook(fb_vid_path, f"{fb_cap}\nLink: {url}")
            time.sleep(10)
