# FILE: main.py
# ROLE: Orchestrator V10.0 (The Intelligent Autonomous Publisher)
# DESCRIPTION: The complete, final, and stable version integrating real-time data, 
#              truth verification, semantic deduplication, and auto-data visualization.
...
def run_pipeline(category, config, forced_keyword=None, is_cluster_topic=False):
    ...
    Integrated with Truth Verification, Data Visualization, and Strict Quality Loops.
    """
    ...
    try:
        ...
        # ======================================================================
        # 5. DATA VISUALIZATION (THE OUT-OF-BOX UPGRADE)
        # ======================================================================
        chart_html_snippet = ""
        try:
            ...
            # Check if we should generate a chart (Heuristic check first)
            if "benchmark" in target_keyword.lower() or "vs" in target_keyword.lower() or "price" in target_keyword.lower() or "score" in target_keyword.lower() or "performance" in target_keyword.lower():
                ...
                if chart_data and chart_data.get('data_points'):
                    ...
                    if chart_url:
                        ...
                        <div class="chart-container" style="margin: 30px 0; text-align: center;">
                            <img src="{chart_url}" alt="{chart_data.get('chart_title')}" style="max-width: 100%; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                            <p style="font-size: 13px; color: #666; margin-top: 5px;">📊 Data Analysis generated by LatestAI</p>
                        </div>
                        '''
                        log(f"      ✅ Chart Generated Successfully: {chart_url}")
        except Exception as e:
